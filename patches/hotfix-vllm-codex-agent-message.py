...
shape with non-empty ``input_text`` content and converts it to a minimal
...
POSTIMAGE_HASHES = {
    "stock-applied": "2e74d1e17c6aa95adcf24d0e3a590b440f7e96bc8838b14b8b513b32aa4d99a4",
    "issue138-applied": "639852638c94667f6fec41876237d32db4cc1943e4c461f49905c4fe726ab1b6",
}

BRANCH_ANCHOR = '''            if item_type == "function_call":
'''

AGENT_BRANCH = '''            agent_message_content = item.get("content")
            agent_message_metadata = item.get(
            ...
            codex_agent_message = (
                ...
                and bool(agent_message_content)
                and all(
                    isinstance(part, dict)
                    and set(part) == {"type", "text"}
                    and part.get("type") == "input_text"
                    and isinstance(part.get("text"), str)
                    for part in agent_message_content
                )
                and (
                    "internal_chat_message_metadata_passthrough" not in item
                    or (
                        isinstance(agent_message_metadata, dict)
                        and set(agent_message_metadata) == {"turn_id", "create_time"}
                        and isinstance(agent_message_metadata.get("turn_id"), str)
                        and bool(agent_message_metadata["turn_id"])
                        and isinstance(
                            agent_message_metadata.get("create_time"), (int, float)
                        )
                        and not isinstance(
