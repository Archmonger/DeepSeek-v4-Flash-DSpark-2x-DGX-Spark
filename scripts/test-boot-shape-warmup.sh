...
#            nocount45 | lateb3 (BATCH_SIZE=3 cache classes appear only on the
#            retry pass; needs STUB_CACHE_DIR).
...
# lateb3 mode: the C=3 sampler bursts only produce their BATCH_SIZE=3 cache
# classes on the RETRY pass (second samp-kp-c3 burst observed), simulating a
# first pass the scheduler split into three B=1 steps.
case "$body" in *" samp-kp-c3-1]"*)
  if [ "${STUB_MODE:-ok}" = "lateb3" ] && [ -n "${STUB_CACHE_DIR:-}" ]; then
    c3_bursts=$(grep -l 'samp-kp-c3-1\]' "$rec"/chat-* 2>/dev/null | wc -l)
    if [ "$c3_bursts" -ge 2 ]; then
      for cls in k:1:0 p:0:1 kp:1:1; do
        d="$STUB_CACHE_DIR/LATEB3${cls%%:*}"
        mkdir -p "$d"
        ku=${cls#*:}; ku=${ku%%:*}; pu=${cls##*:}
        {
          printf 'tt.func public @_topk_topp_kernel(%%LOGITS: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %%K: !tt.ptr<f32>, %%P: !tt.ptr<f32>, %%BATCH_SIZE: i32 loc("BATCH_SIZE")) {\n'
          [ "$ku" = 1 ] && printf '  %%10 = tt.load %%K : !tt.ptr<f32>\n'
          [ "$pu" = 1 ] && printf '  %%11 = tt.load %%P : !tt.ptr<f32>\n'
          printf '  tt.return\n}\n'
...
[ "$k_only" -eq 4 ] && ok "4 k-only sampling requests (samp-k-1 + samp-k-c3 x3)" \
  || bad "k-only sampling requests: $k_only (want 4)"
p_only=$(grep -l '"top_p":0.9' "$rec"/chat-* | xargs -r grep -L '"top_k"' | wc -l)
[ "$p_only" -eq 4 ] && ok "4 p-only sampling requests (samp-p-1 + samp-p-c3 x3)" \
  || bad "p-only sampling requests: $p_only (want 4)"
kp_both=$(grep -l '"top_k":40,"top_p":0.9' "$rec"/chat-* | wc -l)
[ "$kp_both" -eq 4 ] && ok "4 k+p sampling requests (samp-kp-1 + samp-kp-c3 x3)" \
...
if grep -rq 'samp-k-c3\|samp-p-c3\|samp-kp-c3' "$rec_cap2" 2>/dev/null; then
  bad "C=3 sampler burst fired above the MAX_NUM_SEQS=2 profile"
else
  ok "C=3 sampler bursts are skipped at MAX_NUM_SEQS=2 (B=1 combos still fire)"
fi
...
# Fixture entries reproduce the observable axes of real _topk_topp_kernel cache
# TTIR: constexpr combos show up only through argument USE (%K/%P referenced
# beyond their declaration), batch class through the literal %BATCH_SIZE
# declaration form. %PERCENTILE_TO_STD_TABLE is present so a naive %P grep
# without a boundary guard would misclassify every entry as p-using.
mk_fixture_ttir() { # $1 = cache root, $2 = entry dir, $3 = k-used 0/1, $4 = p-used 0/1, $5 = batch: folded|attr|plain
  local d="$1/$2"
  mkdir -p "$d"
  {
    case "$5" in
      folded) printf 'tt.func public @_topk_topp_kernel(%%LOGITS: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %%PERCENTILE_TO_STD_TABLE: !tt.ptr<f32>, %%K: !tt.ptr<f32>, %%P: !tt.ptr<f32>) {\n' ;;
      attr)   printf 'tt.func public @_topk_topp_kernel(%%LOGITS: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %%PERCENTILE_TO_STD_TABLE: !tt.ptr<f32>, %%K: !tt.ptr<f32>, %%P: !tt.ptr<f32>, %%BATCH_SIZE: i32 {tt.divisibility = 16 : i32}) {\n' ;;
      plain)  printf 'tt.func public @_topk_topp_kernel(%%LOGITS: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %%PERCENTILE_TO_STD_TABLE: !tt.ptr<f32>, %%K: !tt.ptr<f32>, %%P: !tt.ptr<f32>, %%BATCH_SIZE: i32 loc("BATCH_SIZE")) {\n' ;;
    esac
...
seed_b1_classes() { # $1 = cache root: the three constexpr combos at BATCH_SIZE=1 (folded)
  mk_fixture_ttir "$1" B1K  1 0 folded
  mk_fixture_ttir "$1" B1P  0 1 folded
  mk_fixture_ttir "$1" B1KP 1 1 folded
}

# 15. all six classes present -> postcondition MET, no retries, exit 0
rec12="$work/r12"; cache12="$work/cache12"
seed_b1_classes "$cache12"
mk_fixture_ttir "$cache12" B3K  1 0 plain
mk_fixture_ttir "$cache12" B3P  0 1 plain
mk_fixture_ttir "$cache12" B3KP 1 1 plain
if run_sweep ok "$rec12" DSPARK_WARMUP_TRITON_CACHE_DIR="$cache12"; then
  ok "postcondition MET with all six signature classes -> exit 0"
else
  bad "sweep exited nonzero despite complete sampler-cache classes"
fi
grep -q "sampler-cache postcondition: MET" "$rec12/stdout" || bad "missing MET line"
met_classes=$(sed -n '/postcondition: MET/,/^[^ ]/p' "$rec12/stdout" | grep -cE '^    (k-only|p-only|k\+p) ')
[ "$met_classes" -eq 6 ] && ok "all six observed classes listed on stdout" \
  || bad "expected 6 listed classes, got $met_classes"
grep -q "re-firing C=3 sampler bursts" "$rec12/stdout" && bad "retry fired despite MET" \
  || ok "no retries when the postcondition is already met"

# 16. B=3 classes never appear -> bounded retries, then UNMET + exit 1;
#     accounting stays exact (retries overwrite result files, tally 47/47)
rec13="$work/r13"; cache13="$work/cache13"
seed_b1_classes "$cache13"
if run_sweep ok "$rec13" DSPARK_WARMUP_TRITON_CACHE_DIR="$cache13"; then
  bad "sweep exited 0 with missing BATCH_SIZE classes"
else
  ok "unmet postcondition fails the (nonfatal) sweep"
fi
retries=$(grep -c "re-firing C=3 sampler bursts" "$rec13/stdout")
[ "$retries" -eq 2 ] && ok "exactly two bounded retries attempted" \
  || bad "retry lines: $retries (want 2)"
grep -q "sampler-cache postcondition UNMET after retries" "$rec13/stderr" \
...
grep -q "^boot-shape-warmup: 47/47 requests ok" "$rec13/stdout" \
  && ok "retries keep the outcome tally exact (47/47)" \
  || bad "tally drifted under retries: $(grep 'requests ok' "$rec13/stdout" || true)"
retry_chats=$(ls "$rec13"/chat-* 2>/dev/null | wc -l)
[ "$retry_chats" -eq 59 ] && ok "retries actually re-fired the C=3 bursts (41 + 2x9 requests)" \
  || bad "recorded chats under retry-exhaustion: $retry_chats (want 59)"

# 17. B=3 classes appear on the retry pass -> one retry, then MET + exit 0
rec14="$work/r14"; cache14="$work/cache14"
seed_b1_classes "$cache14"
if run_sweep lateb3 "$rec14" DSPARK_WARMUP_TRITON_CACHE_DIR="$cache14" STUB_CACHE_DIR="$cache14"; then
  ok "late-arriving B=3 classes -> retry converges, exit 0"
else
  bad "lateb3 sweep exited nonzero"
fi
retries14=$(grep -c "re-firing C=3 sampler bursts" "$rec14/stdout")
[ "$retries14" -eq 1 ] && ok "exactly one retry needed for late B=3 classes" \
  || bad "retry lines: $retries14 (want 1)"
grep -q "sampler-cache postcondition: MET" "$rec14/stdout" \
  && ok "postcondition MET after the converging retry" || bad "missing MET after retry"

# 18. below C=3 the requirement scales down to one class per combo
rec15="$work/r15"; cache15="$work/cache15"
seed_b1_classes "$cache15"
if run_sweep ok "$rec15" DSPARK_WARMUP_TRITON_CACHE_DIR="$cache15" DSPARK_WARMUP_MAX_CONCURRENCY=2; then
  ok "MAX_NUM_SEQS=2: three B=1 classes satisfy the scaled postcondition"
else
  bad "MAX_NUM_SEQS=2 sweep exited nonzero with complete B=1 classes"
fi
grep -q "sampler-cache postcondition: MET" "$rec15/stdout" || bad "missing MET line at C=2"
