...
class ShardAgent:
    ...
    def __init__(
        ...
        self._row_stride = self._slice_bytes * int(world_size)
        ...
        expected = getattr(region, "_row_stride", self._row_stride)
        if expected != self._row_stride:
            raise RuntimeError(
                f"row stride mismatch: region={expected} computed={self._row_stride}"
            )
...
def maybe_start_shard_agent(offloading_spec, handlers) -> None:
    ...
    _AGENT = ShardAgent(
        region=region,
        slice_bytes=offloading_spec.cpu_page_size_per_worker,
