...
- **Boot-shape warmup hardened: authenticated clusters + honest result accounting (follow-up to issue #117)**: `start-deepseek-v4-flash-dspark.sh` now hands the warmup child the same bearer its smoke probe authenticates with — the first already-parsed `DSPARK_API_KEYS` key, else `VLLM_API_KEY` (the two are mutually exclusive upstream) — through a launcher-to-child environment handoff (`DSPARK_WARMUP_BEARER`, not a script argument or log line), and the sweep prefers it over ambient `VLLM_API_KEY`, so keyed clusters no longer 401 away the entire sweep. Every scheduled request is pre-created in the tally before its subshell forks (22 at the shipped `MAX_NUM_SEQS=6` default; 16 at `MAX_NUM_SEQS=4`), so a request whose subshell dies before writing counts as a failed outcome and the summary can never report n/n over fewer outcomes than scheduled. `DSPARK_WARMUP_REQ_TIMEOUT` (per-request curl `--max-time`, default 240 s; sequential worst case ≈ 22 × timeout at the shipped default) is documented in `.env.dspark.example` and `docs/ENVS.md`. The default-on (`DSPARK_BOOT_SHAPE_WARMUP=1`) and launcher-nonfatal contracts are unchanged; CPU behavioral coverage now includes bearer propagation/preference, ambient-key non-leak, open-cluster no-header, and lost-outcome accounting.
