...
- **`NCCL_IB_GID_AUTO=1` FATALed on any list form of `NCCL_IB_HCA`**: `resolve_rocev2_gid_index()` in `start-deepseek-v4-flash-dspark.sh` used the raw `NCCL_IB_HCA` value as a sysfs directory name, so any selector syntax (e.g. the exact list `NCCL_IB_HCA==devA,devB` — recommended filtering on a multi-HCA node, not something `NCCL_IB_MERGE_NICS` itself requires) produced `/sys/class/infiniband/=devA,devB/ports/1/gids/*`, matched nothing, and the launcher exited before starting either rank. The resolver now mirrors NCCL's selector semantics (`net_ib.cc`) on the node that owns the sysfs tree — optional leading `^` exclusion, then optional `=` exact matching, comma-separated `name[:port]` tokens with `atoi()` port parsing, omitted port = every port, tokens transported literally with globbing disabled — and validates **every selected member** against the preferred match IP or an IPv4 on the member's own netdev (one shared match IP no longer silently drops a member on another link address). It fails closed: exit 1 when a selected member has no usable RoCE v2 GID, exit 3 with per-member diagnostics when members need different numeric indexes (`NCCL_IB_GID_INDEX` is one global value per rank, so no single pin can satisfy a disagreeing selection — narrow the selector instead). Single-device configs resolve exactly as before. Gated by `scripts/test-nccl-ib-hca-gid-resolve.sh`: the launcher's own generated lookup script runs against fake sysfs trees and a stubbed `ip`, covering prefix collisions, exclusions/exact exclusions, ports 1 and 2, omitted-port multiport, distinct per-HCA addresses/indexes, disagreement failure, independent head/worker layouts, literal glob/whitespace transport, missing devices, and the original exact-list regression — the suite fails behaviorally (not by extraction) on the pre-fix launcher.
