...
- **Multi-key API auth via `DSPARK_API_KEYS` (default empty = unchanged, unauthenticated)**, with a startup-log redaction hotfix. vLLM's `--api-key` is a dataclass `list[str]`, so one flag carries N space-separated keys; `VLLM_API_KEY` remains the existing single-key option. The two variables are mutually exclusive: when both are set, the container entrypoint and the `start-`/`smoke-`/`status-*.sh` scripts exit 2 naming both variables, before any encoder/hotfix/install work. Key lists are single-line (newline/CR rejected, exit 2), separators trimmed/collapsed, order preserved, duplicates allowed, and `-`-leading tokens rejected; probes use the first parsed key. vLLM `0.25.2.dev0+g752a3a504.d20260714` (Anemll 0.1.1) guards `GUARDED_PREFIX = (/v1, /v2, /inference)` only: `/health`, `/metrics`, `/version`, `/ping` stay keyless, and **`/invocations` is an unguarded inference route** — auth does not protect it, so keyed deployments still need network-level access control on the server port. Keys live in container argv/env (`docker inspect` can read them), so rotation needs a stop/start; when keys are set, `patches/hotfix-vllm-redact-api-key-log.sh` keeps them out of the startup `non-default args` log line (`'api_key': ['<redacted:N value(s)>']`, fail-closed at boot). vLLM does not attribute requests to a key — this is revocation and blast-radius control, not per-user logging. Behavioral gates in `scripts/test-dspark-api-keys.py` (wired into `scripts/ci-validate.sh`) execute the real entrypoint and probe blocks with negative controls for `$$` vs `$` and repeated-flag regressions, plus a real `docker compose config` handoff layer when Docker is available.
