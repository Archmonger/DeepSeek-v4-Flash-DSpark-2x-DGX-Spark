...
- **DSv4 sparse-MLA decode fell back to the heuristic tactic on production dense budgets (PR #83)**: FlashInfer's DSv4 decode tuning config buckets only the *token* axis; the dense / `extra_topk` axis is matched **exactly** against the autotune cache, so every production shape on a dense budget the boot warmup never physically runs is a cache miss served by the C++ closed-form heuristic (`tactic=-1`). `patches/hotfix-dsv4-autotune-prod-dense.py` can merge a pre-generated 24-config supplement (`patches/dsv4-autotune-prod-dense-supp.json`; token axis {1,4,8,16,32,64} × dense {256,1024,2048,8192}) into the boot-tuned cache. It is experimental and default-off; opt in with `DSPARK_ENABLE_DSV4_AUTOTUNE_DENSE_HOTFIX=1`.
  ...
  **Fails closed when enabled**: the patch and exact supplement digest must be present, and the supplement's `_metadata` fingerprint (FlashInfer / CUDA / cuBLAS / cuDNN versions + GPU name) must equal the running build's. Missing or modified inputs, unreadable JSON, absent `_metadata`, anchor drift, a missing boot cache, or an unwritable merged cache stop container startup instead of silently serving the heuristic fallback. Disabled mode never invokes or syncs the patch.

  **Evidence status, stated exactly**: 39 focused CPU tests and the complete `scripts/ci-validate.sh` recipe suite pass. A read-only inspection of the currently deployed stock cache found the same 24 boot-tuned DSv4 entries on both ranks (dense 0/128/512 only), confirming that all shipped supplement budgets are absent there. The contributor's per-kernel microbenchmark remains unverified and is not a served-throughput claim. An exact-head 2× Spark A/B with `DSPARK_ENABLE_DSV4_AUTOTUNE_DENSE_HOTFIX=0` versus `1` is still required before quoting a gain or recommending default-on.
