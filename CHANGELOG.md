...
- **Sampler warmup arms with an observable cache postcondition close the last known mid-serve JIT family (follow-up to issue #117)**: `_topk_topp_kernel` (`vllm/v1/sample/ops/topk_topp_triton.py`) dispatches only when a request carries `top_k`/`top_p`, and every existing sweep arm runs greedy (`temperature: 0`), so the sampler kernel had zero boot coverage — persistent-cache entries born mid-serve (15:26/15:27 UTC, 2026-08-23) prove the gap. Its compile key spans the `TOPK_ENABLED`×`TOPP_ENABLED` constexpr pair plus the `BATCH_SIZE` int specialization, so the sweep now fires all three combos (k-only, p-only, k+p, `temperature: 0.8` — greedy would drop the k/p tensors) at C=1 **and** at C=3, then verifies the result instead of assuming it: a post-sweep check classifies the node's persistent Triton-cache TTIR for `_topk_topp_kernel` (constexpr combos observed through argument *use*, since they are folded out of the signature; the `BATCH_SIZE` class read literally off the declared argument) and requires every combo in two distinct `BATCH_SIZE` classes — so three concurrent curls that the scheduler happens to run as three `BATCH_SIZE=1` steps cannot falsely pass; the C=3 bursts are retried (bounded, accounting-exact) until the classes exist, and a still-missing class fails the non-fatal sweep with a precise diagnostic. The launcher hands the sweep the host path of the node's Triton cache (derived from `HF_CACHE` for the compose-default layout; anything custom skips the check with a note). Honest accounting becomes 47 requests at the shipped `MAX_NUM_SEQS=6` (35 at C=4, 18 at C=2; the C=3 sampler bursts gate on the resolved `MAX_NUM_SEQS` like every other arm). The odd-batch indexer arms this PR originally carried are gone — #135's `do_not_specialize_on_alignment` fix closes `_compute_global_topk_indices_and_lens_kernel` engine-side, which is the deterministic closure request-side arms could only approximate. CPU behavioral coverage: sampler-arm placement and non-leak, C=3 gating at lower profiles, postcondition MET/retry-converges/retry-exhausts/skip paths against fixture TTIR caches, and tally exactness under retries.
