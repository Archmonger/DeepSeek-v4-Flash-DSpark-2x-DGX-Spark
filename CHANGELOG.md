...
- **DSv4 sparse-MLA decode fell back to the heuristic tactic on production dense budgets (PR #83)**: FlashInfer's DSv4 decode tuning config buckets only the *token* axis; the dense / `extra_topk` axis is matched **exactly** against the autotune cache, so every production shape on a dense budget the boot warmup never physically runs is a cache miss served by the C++ closed-form heuristic (`tactic=-1`). `patches/hotfix-dsv4-autotune-prod-dense.py` now merges a pre-generated 24-config supplement (`patches/dsv4-autotune-prod-dense-supp.json`; token axis {1,4,8,16,32,64} × dense {256,1024,2048,8192}) into the boot-tuned cache. Opt out with `DSPARK_SKIP_DSV4_AUTOTUNE_DENSE_HOTFIX=1`. Not gated by `DSPARK_SKIP_HOTFIX`.

  **Where the merge hooks, and why it must be there**: `kernel_warmup.py` owns the DSv4 cache end-to-end — FlashInfer's `sparse_mla_sm120_decode_dsv4_autotune` CM writes the file, the tuning leader reads the bytes back, `world.broadcast_object` ships them to every rank, rank>0 writes its own copy, and then **every** rank calls `AutoTuner.get().load_configs(cache_path)`. vLLM's `write_flashinfer_autotune_cache()` is not on that path at all. The merge therefore runs on the leader *before* the broadcast, and also rewrites the leader's own on-disk cache (upstream never does), so both ranks load an identical config set; merging after the broadcast or on one rank only would leave the two ranks dispatching different kernel tactics. A CI gate pins the rewrite anchor to the vendored `recipe/overlay/.../kernel_warmup.py` and asserts the injection stays pre-broadcast, because the narrow read+broadcast block is byte-identical in the generic `flashinfer_autotune()` and an unscoped anchor would patch the wrong cache.

  **Fails safe, never silently**: the supplement's `_metadata` fingerprint (FlashInfer / CUDA / cuBLAS / cuDNN versions + GPU name) must equal the running build's, or the merge is skipped with a warning naming the mismatched keys rather than loading tactics tuned elsewhere. Missing supplement, unreadable JSON, absent `_metadata`, or an unwritable cache all leave the boot-tuned cache untouched. Anchor drift makes the patcher exit non-zero with an explicit message instead of reporting success. `start-deepseek-v4-flash-dspark.sh` refuses to start if only one of the `.py`/`.json` pair is present, since a half-synced pair would give the worker rank different coverage from the head.

  **Evidence status, stated exactly**: verification here is **CPU/static only** — 31 unit tests in `scripts/test-dsv4-autotune-prod-dense.py` covering anchor uniqueness and pinning, pre-broadcast injection, `ast` validity, idempotence, both-ranks-identical config sets, boot-tuned-wins-on-collision, and every fail-safe branch, plus the `scripts/ci-validate.sh` gates. The contributor's `+20%`–`+82%` per-kernel-call and `~+29-33%` traffic-weighted figures are a **microbenchmark of heuristic vs tuned `chunks_per_block` cost on the reported shapes, measured outside this repo and not reproduced here**. **No served-throughput claim is made**: there is no end-to-end decode A/B for this change, and the first revision of PR #83 patched `write_flashinfer_autotune_cache`, which is not on the DSv4 path, so no prior measurement can be attributed to the shipped mechanism either. A 2× Spark decode A/B with the hotfix on vs. `DSPARK_SKIP_DSV4_AUTOTUNE_DENSE_HOTFIX=1` is required before any tok/s number is quoted.
