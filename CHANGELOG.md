...
- **Boot-shape warmup hardened: authenticated clusters + honest result accounting (follow-up to issue #117)**: `start-deepseek-v4-flash-dspark.sh` now hands the warmup child the same bearer its smoke probe authenticates with — the first already-parsed `DSPARK_API_KEYS` key, else `VLLM_API_KEY` (the two are mutually exclusive upstream) — through a launcher-to-child environment handoff (`DSPARK_WARMUP_BEARER`, not a script argument or log line), and the sweep prefers it over ambient `VLLM_API_KEY`, so keyed clusters no longer 401 away the entire sweep. Every one of the 16 scheduled requests is pre-created in the tally before its subshell forks, so a request whose subshell dies before writing counts as a failed outcome and the summary can never report n/n over fewer outcomes than scheduled. `DSPARK_WARMUP_REQ_TIMEOUT` (per-request curl `--max-time`, default 240 s; sequential worst case ≈ 16 × timeout) is documented in `.env.dspark.example` and `docs/ENVS.md`. The default-on (`DSPARK_BOOT_SHAPE_WARMUP=1`) and launcher-nonfatal contracts are unchanged; CPU behavioral coverage now includes bearer propagation/preference, ambient-key non-leak, open-cluster no-header, and lost-outcome accounting.
- **Bucket-ladder correction to the boot-shape sweep (follow-up to issue #117, review-driven)**: the #117 sweep materialized only BLOCK {16, 256} of `_prepare_dflash_inputs_kernel`, while the installed kernel's live Triton cache holds six compile keys {8, 16, 32, 64, 128, 256}; the mechanism review established that request concurrency does not enter this kernel's compile key (`BLOCK_SIZE = min(256, next_pow2(scheduled_tokens + 6))`), so chat-batch arms cannot reach the low buckets and the old "small prefill tails fill the low buckets" claim was false (longchunk tail ≈1342 tokens → BLOCK 256). The sweep now adds a deterministic plain-completion bucket ladder — exact-token prompts s = {1, 6, 20, 45, 100, 200} via `POST /v1/completions`, `next_pow2(s+6)` mapping onto all six live keys — with every rung verified at runtime through an authenticated `POST /tokenize` before firing; a count mismatch (or tokenize error) skips the rung and fails the nonfatal warmup with a precise, secret-free diagnostic. C=6 is removed (deployed nodes run `--max-num-seqs 4`, so C=6 was never schedulable); C=1/2/4 + medium/long-prefill + thinking-off arms remain for batch-keyed kernels. CPU behavioral matrix grows 19 → 43 checks; the new suite fails against the pre-fix sweep (29 failures) and is green after it.
