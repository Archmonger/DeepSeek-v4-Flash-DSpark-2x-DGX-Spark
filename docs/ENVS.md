...
| `DSPARK_ENABLE_DSV4_AUTOTUNE_DENSE_HOTFIX` | `1` opts into the experimental PR #83 production-dense supplement; default `0` leaves the boot-tuned DSv4 sparse-MLA decode cache unchanged. Enabled mode merges the pinned 24-config supplement on the tuning leader before `broadcast_object`, then fails boot if the patch, supplement digest, running-build fingerprint (FlashInfer/CUDA/cuBLAS/cuDNN + GPU), or cache installation is invalid. |
