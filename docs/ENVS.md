...
| `DSPARK_WORKER_HF_NFS` | Launcher-side. `1` (default): worker mounts the head HuggingFace cache over NFSv4 on `NCCL_SOCKET_IFNAME` (ConnectX). Hub weights are not copied to the worker. JIT dirs (`triton-cache`, `tilelang-cache`, `vllm-cache`, `flashinfer`, `b12x-cute-cache`, `nccl-fr`) are local overlays under `WORKER_HF_CACHE`. Reuses a live NFSv4 exporter on that address (e.g. Qwen `vllm-fn-nfs`). `0`: bind a local worker checkpoint (`prepare` downloads on both nodes). |
