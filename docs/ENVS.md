...
| `DSPARK_API_KEYS` | **Optional multi-key auth**, enforced by vLLM itself. Single-line space-separated keys flattened into **one** `--api-key` flag (nargs list; repeating the flag would overwrite, so a per-key loop is invalid). One key per caller lets you revoke a single key. Empty/whitespace-only (default) = no `--api-key` at all, stock behavior unchanged. Parsing trims/collapses separators, preserves order, allows duplicates, and rejects tokens starting with `-` (exit 2); newline/CR in the value is rejected (exit 2) rather than silently truncating the list. **Mutually exclusive with `VLLM_API_KEY`**: if both are set, the container entrypoint and the `start-`/`smoke-`/`status-*.sh` scripts exit 2 naming both variables, before any patch/install work. On the pinned image (vLLM `0.25.2.dev0+g752a3a504.d20260714`), `GUARDED_PREFIX = (/v1, /v2, /inference)`: `/health`, `/metrics`, `/version`, `/ping` stay keyless, and **`/invocations` is an unguarded inference route** — auth does not protect it, so a keyed deployment still needs network-level access control on the server port. Keys are container argv/env (`docker inspect` / host `ps` can read them), so rotation needs a stop/start; with keys set, `patches/hotfix-vllm-redact-api-key-log.sh` keeps them out of the startup `non-default args` log line. vLLM does not attribute requests to a key — this is revocation and blast-radius control, not per-user logging. |
| `patches/hotfix-vllm-redact-api-key-log.sh` | Key-log redaction hotfix, applied by the container entrypoint's hotfix loop (no env toggle). Upstream `log_non_default_args()` prints every `--api-key` value verbatim in the startup `non-default args` line; the patch redacts in that logger for both the api_server and llm entrypoints, preserving the count — the line shows `'api_key': ['<redacted:N value(s)>']` (N = number of keys) — so the single-`--api-key`-flag diagnostic still proves N keys landed. Closes the *log* channel only: keys stay readable via `docker inspect` / host `ps` (argv/env exposure by design). Enable it on any keyed deployment; `bash patches/hotfix-vllm-redact-api-key-log.sh --status` (inside the container) reports applied state. |
