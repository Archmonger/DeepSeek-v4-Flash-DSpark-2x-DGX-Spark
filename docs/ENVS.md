...
| `DSPARK_ISSUE117_DISPATCH_TRACE` | Experimental, default `0`. `1` applies the source-locked issue #117 executor trace at container boot and emits `[issue117-dispatch-trace]` records for driver dispatch/response and each worker's receive/call/response phases; any source drift fails startup rather than running partial instrumentation. For the controlled test, enable it on both members, run identical traffic after a clean paired launch and after the worker-only dirty restart, and archive both ranks' logs. `disp` without matching `recv` isolates dispatch delivery; `recv` without `done`/`fail` isolates worker execution; `done` without `resp` isolates async device-output wait; `resp` without driver `resp_ok`/`resp_timeout` isolates response transport. |
