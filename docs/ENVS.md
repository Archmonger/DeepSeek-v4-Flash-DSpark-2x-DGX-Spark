...
| `TORCH_NCCL_TRACE_BUFFER_SIZE` | **Not** `VLLM_*`. Compose default `2000` (torch 2.11 default, pinned). Ring-buffer size of PyTorch's ProcessGroupNCCL flight recorder; `>0` is required for dump-on-timeout and the pipe trigger below. |
