...
build_one() {
  ...
  if [ "$host" = "local" ]; then
    ...
    docker run --rm --entrypoint /opt/env/bin/python "$DSPARK_BASE_IMAGE" -c \
      "import vllm.v1.spec_decode.dspark as d; import vllm.v1.spec_decode.dspark_proposer as p; assert d.map_dspark_stacked_param_name('model.layers.0.ffn.shared_experts.w1.weight') == ('model.layers.0.ffn.shared_experts.gate_up_proj.weight', 0); assert d.resolve_drafter_capture_sizes('1', 6) == [1, 2, 4, 6]; print('dspark overlay ok', d.__name__, p.__name__)"
    docker build \
